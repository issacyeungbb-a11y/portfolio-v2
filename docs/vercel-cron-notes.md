...
`vercel.json` 目前只有每日更新相關 cron；季度報告明確只可手動生成。

如日後需要保守加入，香港時間與 UTC 轉換建議如下：

- Monthly analysis: 香港時間每月 1 日 08:15
  - UTC cron: `15 0 1 * *`
- Quarterly report: 不設 cron；由分析頁手動按「生成季報」觸發。

現有 `hasExistingMonthlyAnalysis()` / `hasExistingQuarterlyReport()` 已可避免手動重複生成，但正式啟用前仍應先驗證：
