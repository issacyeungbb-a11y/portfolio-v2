...
import type {
  AllocationBucketKey,
  ReportAllocationBasis,
  ReportAllocationDeltaSummary,
  ReportAllocationSliceSummary,
  ReportAllocationStyleTag,
  ReportAllocationSummary,
} from '../../types/portfolio';

export interface ReportAllocationHoldingInput {
  assetType?: AllocationBucketKey | string;
  currency?: string;
  marketValue?: number;
...
interface BuildReportAllocationSummaryParams {
  holdings: ReportAllocationHoldingInput[];
  asOfDate: string;
  basis: ReportAllocationBasis;
  comparisonHoldings?: ReportAllocationHoldingInput[] | null;
  comparisonLabel?: string;
}

function isAllocationBucketKey(value: unknown): value is AllocationBucketKey {
  return (
    value === 'stock' ||
    value === 'etf' ||
    value === 'bond' ||
...
function getInputValueHKD(holding: ReportAllocationHoldingInput) {
...
function getSlicePercentage(
  slices: Array<Pick<ReportAllocationSliceSummary, 'key' | 'percentage'>>,
  key: AllocationBucketKey,
) {
  return slices.find((slice) => slice.key === key)?.percentage ?? 0;
}

function buildReportAllocationSlices(
  holdings: ReportAllocationHoldingInput[],
): ReportAllocationSliceSummary[] {
  const buckets = new Map<AllocationBucketKey, number>();
  ...
  return allocationBucketOrder
    .map((key) => {
      ...
      return {
        ...
        color: meta.color,
        percentage:
          totalValueHKD === 0 ? 0 : (totalValueHKDForBucket / totalValueHKD) * 100,
...
export function buildAllocationDeltas(
  currentSlices: Array<Pick<ReportAllocationSliceSummary, 'key' | 'percentage'>>,
  previousSlices: Array<Pick<ReportAllocationSliceSummary, 'key' | 'percentage'>>,
): ReportAllocationDeltaSummary[] {
  const keys = allocationBucketOrder.filter(
    (key) =>
      currentSlices.some((slice) => slice.key === key) ||
  ...
  return keys.map((key) => ({
    ...
    deltaPercentagePoints:
      getSlicePercentage(currentSlices, key) - getSlicePercentage(previousSlices, key),
  }));
}

export function deriveAllocationStyleTag(
  slices: Array<Pick<ReportAllocationSliceSummary, 'key' | 'percentage'>>,
): ReportAllocationStyleTag {
  ...
  const stockAndEtf =
    getSlicePercentage(slices, 'stock') + getSlicePercentage(slices, 'etf');
...
export function deriveAllocationWarningTags(
  slices: Array<Pick<ReportAllocationSliceSummary, 'key' | 'percentage'>>,
): string[] {
  const sorted = [...slices].sort((left, right) => right.percentage - left.percentage);
  const largest = sorted[0]?.percentage ?? 0;
...
function getStylePhrase(styleTag: ReportAllocationStyleTag) {
...
function buildSummarySentence(
  ...
  const leadingPhrase = dominantSlice
    ...
    : '目前組合未有有效資產分佈';
  const warningPhrase =
    warningTags.find((tag) => tag.includes('現金')) ??
...
export function buildReportAllocationSummaryFromHoldings(
  params: BuildReportAllocationSummaryParams,
): ReportAllocationSummary {
  const slices = buildReportAllocationSlices(params.holdings);
  const previousSlices =
    params.comparisonHoldings && params.comparisonHoldings.length > 0
      ? buildReportAllocationSlices(params.comparisonHoldings)
      : [];
  const deltas =
    previousSlices.length > 0 ? buildAllocationDeltas(slices, previousSlices) : undefined;
  ...
  const summary: ReportAllocationSummary = {
...
function normalizeSliceSummary(value: unknown): ReportAllocationSliceSummary | null {
...
function normalizeDeltaSummary(value: unknown): ReportAllocationDeltaSummary | null {
  if (typeof value !== 'object' || value === null) {
    return null;
  }
...
function isReportAllocationStyleTag(value: unknown): value is ReportAllocationStyleTag {
  return (
    value === '平衡型' ||
    value === '進攻型' ||
    value === '防守型' ||
...
function isReportAllocationBasis(value: unknown): value is ReportAllocationBasis {
  return value === 'monthly' || value === 'quarterly';
}

export function normalizeReportAllocationSummary(
  value: unknown,
): ReportAllocationSummary | undefined {
  if (typeof value !== 'object' || value === null) {
    return undefined;
  }
  ...
  const slices = Array.isArray(raw.slices)
    ? raw.slices.map(normalizeSliceSummary).filter((slice): slice is ReportAllocationSliceSummary => slice !== null)
    : [];
  ...
  const deltas = Array.isArray(raw.deltas)
    ? raw.deltas.map(normalizeDeltaSummary).filter((delta): delta is ReportAllocationDeltaSummary => delta !== null)
    : undefined;
  ...
  return {
    ...
    basis: raw.basis,
    comparisonLabel:
      typeof raw.comparisonLabel === 'string' && raw.comparisonLabel.trim()
    ...
    totalValueHKD: toFiniteNumber(raw.totalValueHKD),
    summarySentence:
      typeof raw.summarySentence === 'string' && raw.summarySentence.trim()
