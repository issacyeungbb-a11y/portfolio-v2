...
function isPublicPortfolioRequest(request: ApiRequest) {
  const requestUrl = request.url ?? '/api/health';
...
function getConfiguredPublicPortfolioCode() {
  return (
...
function getPublicPortfolioQueryCode(request: ApiRequest) {
  try {
    return new URL(request.url ?? PUBLIC_PORTFOLIO_ROUTE, 'http://localhost').searchParams.get('code')?.trim() ?? '';
  } catch {
    return '';
  }
}
...
function isArchivedAsset(value: Record<string, unknown>) {
...
async function fetchPublicAssetDocuments(projectId: string) {
