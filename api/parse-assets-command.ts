...
} from '../server/parseAssetsCommand.js';
import {
...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  const route = '/api/parse-assets-command';
  ...
  try {
    ...
    const result = await parseAssetsFromCommand(payload);
    ...
    const formatted = getParseAssetsCommandErrorResponse(error);
