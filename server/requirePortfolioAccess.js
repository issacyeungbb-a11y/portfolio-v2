...
function getConfiguredPortfolioAccessCode() {
    return (process.env.PORTFOLIO_ACCESS_CODE?.trim() ||
...
export function isPortfolioAccessError(error) {
    return error instanceof PortfolioAccessError;
}
export async function requirePortfolioAccess(request, route) {
    const configuredCode = getConfiguredPortfolioAccessCode();
    if (!configuredCode) {
        throw new PortfolioAccessError('尚未設定共享存取碼，請先設定 PORTFOLIO_ACCESS_CODE 或 VITE_PORTFOLIO_ACCESS_CODE。', route, 500);
    }
...
export function getPortfolioAccessErrorResponse(error, route) {
