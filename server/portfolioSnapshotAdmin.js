...
function normalizeAssetInput(value) {
  const lastPriceUpdatedAt = value.lastPriceUpdatedAt instanceof Timestamp ? value.lastPriceUpdatedAt.toDate().toISOString() : typeof value.lastPriceUpdatedAt === "string" ? value.lastPriceUpdatedAt : void 0;
  const priceAsOf = value.priceAsOf instanceof Timestamp ? value.priceAsOf.toDate().toISOString() : typeof value.priceAsOf === "string" ? value.priceAsOf : void 0;
  const archivedAt = value.archivedAt instanceof Timestamp ? value.archivedAt.toDate().toISOString() : typeof value.archivedAt === "string" ? value.archivedAt : void 0;
  return {
    name: typeof value.name === "string" ? value.name : "",
    symbol: typeof value.symbol === "string" ? value.symbol : "",
    assetType: normalizeAssetType(value.assetType),
    accountSource: value.accountSource === "Futu" || value.accountSource === "IB" || value.accountSource === "Crypto" || value.accountSource === "Other" ? value.accountSource : "Other",
...
async function readPersistedFxRates(maxAgeMs = 24 * 60 * 60 * 1e3) {
  try {
    const db = getFirebaseAdminDb();
    const docSnap = await db.collection(SHARED_PORTFOLIO_COLLECTION).doc(SHARED_PORTFOLIO_DOC_ID).get();
    const data = docSnap.data()?.fxRates;
    if (!data) return null;
    const updatedAtRaw = data.updatedAt;
    const updatedAt = typeof updatedAtRaw === "string" ? new Date(updatedAtRaw) : updatedAtRaw instanceof Timestamp ? updatedAtRaw.toDate() : null;
...
async function captureAdminPortfolioSnapshot(params) {
  const db = getFirebaseAdminDb();
  const snapshotId = params.snapshotId?.trim();
  validateDailySnapshotId(snapshotId);
  const snapshotRef = db.collection(SHARED_PORTFOLIO_COLLECTION).doc(SHARED_PORTFOLIO_DOC_ID).collection("portfolioSnapshots").doc(snapshotId);
  if (!params.force) {
  ...
  const holdingsPayload = holdings.map((holding) => ({
    ...
    priceAsOf: holding.lastPriceUpdatedAt ?? null,
    // P2-6: 追溯價格時間
...
export {
  captureAdminPortfolioSnapshot,
  readAdminPortfolioAssets,
  readPersistedFxRates
};
