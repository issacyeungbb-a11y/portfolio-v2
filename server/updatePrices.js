...
function getGeminiApiKey() {
    ...
    if (!apiKey) {
        throw new UpdatePricesError('未設定 GEMINI_API_KEY 或 GOOGLE_API_KEY，暫時無法執行 AI 價格更新。', 500);
    }
    return apiKey;
}
...
function isLikelyHongKongTicker(ticker, currency) {
    const normalizedTicker = ticker.trim().toUpperCase();
    const normalizedCurrency = currency.trim().toUpperCase();
    return (normalizedCurrency === 'HKD' &&
        (/^\d{4,5}$/.test(normalizedTicker) || /^\d{4,5}\.HK$/.test(normalizedTicker)));
}
function buildHongKongTickerVariants(ticker) {
...
function sanitizeSinglePriceUpdateResult(rawPayload) {
    if (typeof rawPayload !== 'object' ||
        rawPayload === null ||
...
function createFailedModelResult(asset, reason) {
    return {
...
function buildInvalidReason(category) {
    if (category === 'ticker_format')
        return '代號格式可能有問題，AI 未能準確對應市場報價';
    ...
    if (category === 'response_format')
        return '模型回覆格式不正確，未能穩定解析';
    if (category === 'price_missing')
        return 'AI 未取得有效最新價格';
    if (category === 'confidence_low')
        return '可信度不足，暫不自動套用';
    if (category === 'diff_too_large')
        return '價格差距過大，需要人工檢查';
    return '未能自動更新，請再檢查';
}
function tryParseSingleModelResult(asset, rawText, reason) {
    try {
...
function buildAssetSearchHints(asset) {
    const hints = [
...
function buildSingleAssetPrompt(asset, mode) {
    const extraRetryRules = mode === 'retry'
        ? `
...
async function generateSingleAssetPriceResponse(ai, model, prompt) {
    try {
        return await ai.models.generateContent({
            model,
            contents: prompt,
...
function isUsableModelResult(asset, result) {
    const minimumConfidence = getMinAutoApplyConfidence();
...
async function generateBestPriceForAsset(ai, model, asset) {
    ...
    const primaryResult = tryParseSingleModelResult(asset, primaryResponse.text ?? '', 'Gemini primary 回覆格式不正確');
    ...
    const retryResult = tryParseSingleModelResult(asset, retryResponse.text ?? '', 'Gemini retry 回覆格式不正確');
...
function buildReviewResults(requestedAssets, modelResults) {
    ...
    return requestedAssets.map((asset, index) => {
        ...
        const matched = modelResults.find((item) => item.ticker?.toUpperCase() === asset.ticker.toUpperCase() ||
            item.assetName?.toLowerCase() === asset.assetName.toLowerCase()) ?? modelResults[index];
        ...
        const diffPct = nextPrice != null && asset.currentPrice > 0
            ...
            : 0;
        const forcedNeedsReview = nextPrice == null ||
            nextPrice <= 0 ||
            staleQuote ||
            diffPct >= threshold ||
...
async function generatePriceUpdatesForBatch(ai, model, assets) {
    ...
    const missingOrWeakAssets = assets.filter((asset) => {
        const matched = sanitizedResults.find((item) => item.ticker?.toUpperCase() === asset.ticker.toUpperCase() ||
    ...
    if (missingOrWeakAssets.length > 0) {
        const focusedResults = await Promise.all(missingOrWeakAssets.map(async (asset) => {
            try {
        ...
        sanitizedResults = [
            ...sanitizedResults.filter((item) => !missingOrWeakAssets.some((asset) => item.ticker?.toUpperCase() === asset.ticker.toUpperCase() ||
                item.assetName?.toLowerCase() === asset.assetName.toLowerCase())),
            ...focusedResults,
        ];
    }
...
export async function generatePriceUpdates(payload) {
    ...
    for (let index = 0; index < request.assets.length; index += PRICE_UPDATE_BATCH_SIZE) {
        ...
        try {
            ...
            batchResults = buildReviewResults(assetBatch, assetBatch.map((asset) => createFailedModelResult(asset, 'Gemini 批次回覆格式不正確')));
        }
