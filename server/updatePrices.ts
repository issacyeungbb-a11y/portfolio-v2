...
const AI_PRICE_FALLBACK_MODEL =
  process.env.AI_PRICE_FALLBACK_MODEL?.trim() ||
  process.env.GROUNDED_GEMINI_MODEL?.trim() ||
  'gemini-2.5-flash';
const yahooFinanceClient = new YahooFinance();

const LEGACY_COIN_GECKO_ID_OVERRIDES: Record<string, { coinId: string }> = {
...
interface CoinGeckoIdOverrideEntry {
  ticker: string;
  coinId: string;
  coinSymbol: string;
  coinName: string;
...
function normalizeCoinGeckoOverrideEntry(
  ticker: string,
  value: Record<string, unknown>,
): CoinGeckoIdOverrideEntry | null {
...
function createCacheEntryFromOverride(
  ticker: string,
  override: CoinGeckoIdOverrideEntry,
): CoinGeckoCoinIdCacheEntry {
...
const coinGeckoOverrideMemoryCache = new Map<string, CoinGeckoIdOverrideEntry>();

async function readCoinGeckoOverrideEntry(ticker: string) {
  const normalizedTicker = normalizeCoinGeckoTicker(ticker);
...
async function readCoinGeckoOverrideEntries(tickers: string[]) {
  if (tickers.length === 0) {
    return new Map<string, CoinGeckoIdOverrideEntry>();
  }

  const normalizedTickers = [...new Set(tickers.map(normalizeCoinGeckoTicker))];
  const overrideEntries = new Map<string, CoinGeckoIdOverrideEntry>();
  ...
  try {
    ...
    snapshots.forEach((snapshot, index) => {
      ...
      const override = normalizeCoinGeckoOverrideEntry(ticker, snapshot.data() as Record<string, unknown>);
      if (override) {
...
function getFirstGroundingSourceUrl(response: unknown) {
  const candidates = (response as Record<string, unknown>)?.candidates;
  ...
  for (const chunk of chunks) {
    const web =
      typeof chunk === 'object' && chunk !== null
        ? (chunk as Record<string, unknown>).web as Record<string, unknown> | undefined
        : undefined;
...
async function generateGroundedGeminiJson(prompt: string, apiKey: string) {
...
function buildAiPriceFallbackPrompt(asset: PriceUpdateRequestAsset, symbol: string) {
...
function normalizeAiPriceFallback(
  asset: PriceUpdateRequestAsset,
  symbol: string,
  payloadText: string,
  groundingSourceUrl: string | null,
): MarketPriceResult | null {
  try {
    const parsed = JSON.parse(extractJsonObject(payloadText).trim()) as Record<string, unknown>;
...
async function fetchAiPriceFallback(
  asset: PriceUpdateRequestAsset,
...
async function applyAiPriceFallbackForMissingYahooResults(
  assets: PriceUpdateRequestAsset[],
  results: MarketPriceResult[],
): Promise<MarketPriceResult[]> {
  ...
  const missingResults = results
    ...
    .filter((asset): asset is PriceUpdateRequestAsset => Boolean(asset))
  ...
  const fallbackByAssetId = new Map<string, MarketPriceResult>();
