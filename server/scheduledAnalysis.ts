...
export function getDefaultServerPromptSettings(): AnalysisPromptSettings {
  return {
    asset_analysis: [
      ...
      '2. 【本月資產變化摘要】（以 4-6 個「- 」開頭的短點輸出，每點一行，必須區分總資產變化、淨入金 / 出金、扣除資金流後表現、資金流覆蓋率、最大貢獻者 / 最大拖累者，並判斷升幅是否集中於少數高 beta / 加密 / 科技資產）',
      '3. 【組合健康檢查】（以「- 」列點做宏觀壓力測試：risk-on 持續時如何受惠、risk-off 時最大風險、現金 / 債券是否足夠防守、USD / HKD / JPY 幣別曝險是否需要留意；如果 dataQualitySummary.status 不是 ok，要限制結論強度）',
    ...
    asset_report: [
      ...
      '涉及多個重點的段落，每個重點獨立一行並以「- 」開頭；限制提示用全形括號（）包住獨立成行。',
    ].join('\n'),
  };
}
...
/**
 * 排版格式規則 — 前端 ReportBody 會將列點、分類標籤、箭咀流程、
 * 括號備註解析成結構化排版；呢度嘅格式約定必須同 reportRichText.ts 一致。
 */
const REPORT_TEXT_FORMAT_RULES = [
  '排版格式規則（必須遵守，系統會按此做結構化排版）：',
  '涉及多個重點的段落，每個重點獨立一行並以「- 」開頭；不要用分號把多個重點串成一句長句。',
  '每句盡量不超過 40 字；先講結論，再講原因。',
  '金額寫法統一為「數字 + 幣別」（例如 -7.5 萬 HKD、USD 234,008）；升跌一律帶正負號（+28.3% / -10.7%）。',
  '列點內如有分類或主題，用「主題：內容」格式開頭（例如「Risk-off 情境：…」），主題保持精簡。',
  '限制提示或備註用全形括號（）包住整段，獨立成行放在該段最後。',
].join('\n');
...
export function buildMonthlyAnalysisQuestion(params: {
  ...
  return [
    ...
    '【本月資產變化摘要】（以 4-6 個「- 」開頭的短點輸出，每點一行：a. 總資產變化＋淨入金／出金＋資金流覆蓋率；b. 最大拖累者及金額；c. 最大貢獻者及金額；d. 回報是否集中於少數高 beta / 加密 / 科技資產；e. 倉位調整或資料雜訊提醒（如有））',
    '【組合健康檢查】（開頭可用一句交代資料品質，之後以「- 」列點：Risk-on 情境、Risk-off 情境、防守能力、幣別曝險各一點；如資料品質有限要主動收窄結論）',
...
function buildQuarterlyAnalysisQuestion(
  ...
  return [
    ...
    '【幣別曝險】',
    '【重點持倉分析】',
    '【季度對比摘要】（以「- 」列點交代：總資產變化、淨入金／出金、扣除資金流後變化；如資料不足要明確講限制）',
    '【主要風險與集中度】（以「- 」列點，每點一個風險，用「主題：內容」格式開頭）',
    '【下季觀察重點】（以「- 」列點，每點一行並帶量化觸發條件）',
    '',
    macroSummaryText,
    '',
    '規則：',
    ...
    '短而準，繁體中文輸出；資料不足就直說。',
    '【管理層摘要】限 3 句以內，突出本季最重要的變化、歸因與風險。',
    '',
    REPORT_TEXT_FORMAT_RULES,
    '',
