...
const GEMINI_GROUNDING_TIMEOUT_MS = 8_000;
const GEMINI_GROUNDING_TOTAL_BUDGET_MS = 12_000;
const GEMINI_ANALYSIS_TIMEOUT_MS = 32_000;
const CLAUDE_ANALYSIS_TIMEOUT_MS = 32_000;
const QUALITY_REWRITE_TIMEOUT_MS = 12_000;
...
export interface AnalyzePortfolioTestHooks {
  generateExternalSearchSummary?: (
    request: PortfolioAnalysisRequest,
    intent: AnalysisIntent,
  ) => Promise<ExternalSearchResult>;
  buildEarningsEvidencePack?: typeof buildEarningsEvidencePack;
  qualityCheckGeneralAnswer?: typeof qualityCheckGeneralAnswer;
  analyzeWithGemini?: typeof analyzeWithGemini;
  analyzeWithClaude?: typeof analyzeWithClaude;
}
...
function normalizeStructuredGeneralAnswerText(answer: string, depth = 0): string {
  ...
  try {
    ...
    // Not a nested JSON answer. Keep the text as-is.
  }
...
export function seedExternalEvidenceCacheForTest(
  request: PortfolioAnalysisRequest,
  intent: AnalysisIntent,
  value: ExternalSearchResult,
  ttlMs = 60 * 60 * 1000,
) {
...
function getEarningsSourcePriority(sourceType: ExternalEvidenceSourceType) {
  switch (sourceType) {
...
function sortEarningsSourcesByPriority(sources: ExternalEvidenceSource[]) {
  return [...sources].sort(
    (left, right) => getEarningsSourcePriority(left.sourceType) - getEarningsSourcePriority(right.sourceType),
  );
}

function getPreferredEarningsSources(sources: ExternalEvidenceSource[]) {
  const sorted = sortEarningsSourcesByPriority(sources);
  const official = sorted.filter((source) =>
    ['official_report', 'sec_filing', 'company_ir', 'earnings_call'].includes(source.sourceType),
  );
  return official.length > 0 ? official : sorted;
}

function findEarningsFigure(
  sources: ExternalEvidenceSource[],
  patterns: RegExp[],
) {
...
function addMissingEarningsFieldUncertainty(
  uncertainty: string[],
  fieldName: keyof EarningsEvidencePack,
...
function completeEarningsEvidencePack(
  basePack: EarningsEvidencePack,
  externalSources: ExternalEvidenceSource[],
): EarningsEvidencePack {
  ...
  const pick = (current: string | null, patterns: RegExp[]) =>
    current ?? findEarningsFigure(sortedSources, patterns);
  const segmentRevenue =
    basePack.segmentRevenue.length > 0
      ...
      : getPreferredEarningsSources(sortedSources)
          ...
          .slice(0, 8);
  const segmentOperatingIncome =
    basePack.segmentOperatingIncome.length > 0
      ...
      : getPreferredEarningsSources(sortedSources)
          ...
          .slice(0, 8);
  const completed: EarningsEvidencePack = {
    ...basePack,
...
function mergeEarningsEvidencePacks(
  parsedPack: EarningsEvidencePack | undefined,
  deterministicPack: EarningsEvidencePack,
) {
  ...
  return completeEarningsEvidencePack(
    {
      ...
      segmentRevenue:
        parsedPack.segmentRevenue.length > 0 ? parsedPack.segmentRevenue : deterministicPack.segmentRevenue,
      segmentOperatingIncome:
        parsedPack.segmentOperatingIncome.length > 0
          ? parsedPack.segmentOperatingIncome
          : deterministicPack.segmentOperatingIncome,
      managementCommentary:
        parsedPack.managementCommentary.length > 0
          ? parsedPack.managementCommentary
          : deterministicPack.managementCommentary,
      marketReaction:
        parsedPack.marketReaction.length > 0 ? parsedPack.marketReaction : deterministicPack.marketReaction,
      oneOffItems:
        parsedPack.oneOffItems.length > 0 ? parsedPack.oneOffItems : deterministicPack.oneOffItems,
...
function ensureEarningsEvidencePack(
  searchResult: ExternalSearchResult,
  request: PortfolioAnalysisRequest,
  buildPack: typeof buildEarningsEvidencePack,
    ...
    for (const model of candidates) {
      ...
      try {
        const response = await generateGeminiContentViaRest({
          ...
          googleSearch: true,
          timeoutMs: Math.min(GEMINI_GROUNDING_TIMEOUT_MS, Math.max(2_000, remainingMs)),
        });
